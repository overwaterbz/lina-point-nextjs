...
export interface WhatsAppAgentInput {
  ...
  /** Last N guest memories loaded from guest_memory table */
  guestMemories?: string[];
}
...
async function buildSystemPrompt(
  ...
  const memoriesStr =
    guestMemories && guestMemories.length > 0
      ? `\n\nGUEST MEMORY (from previous interactions):\n${guestMemories.map((m) => `- ${m}`).join("\n")}`
      : "";

  return `${basePrompt}

GUEST PREFERENCES: ${JSON.stringify(prefs)}${memoriesStr}
...
- Reference guest memory when relevant to personalise responses.
...
function extractBookingDetails(
  ...
  if (isoDates.length >= 2 && !data.checkOutDate)
    data.checkOutDate = isoDates[1];
  ...
  const groupMatch = message.match(
    /(\d+)\s+(guests?|people|adults?|persons?)/i,
  );
  if (groupMatch && !data.groupSize) data.groupSize = Number(groupMatch[1]);

  const lower = message.toLowerCase();
  if (lower.includes("overwater") || lower.includes("cabana"))
    data.roomType = "overwater cabana";
  if (lower.includes("villa") || lower.includes("beach villa"))
    data.roomType = "overwater cabana";
  if (lower.includes("2nd floor") || lower.includes("second floor"))
    data.roomType = "2nd floor suite";
  if (
    lower.includes("1st floor") ||
    lower.includes("first floor") ||
    lower.includes("hotel suite")
  )
    data.roomType = "1st floor suite";
  if (lower.includes("suite") && !data.roomType)
    data.roomType = "2nd floor suite";
...
export async function runWhatsAppConciergeAgent(
  ...
  const graph = new StateGraph(ConciergeState)
    .addNode("respond", async (state) => {
      const systemPrompt = await buildSystemPrompt(
        state.profile,
        state.refinementHint,
        input.guestMemories,
      );
      const history = state.sessionContext.messages
        .slice(-5)
        .map((msg) =>
          msg.role === "user"
  ...
  const { result } = await runWithRecursion(
    ...
    async (state) => {
      const goal =
        "Provide a short, friendly WhatsApp reply that moves the conversation forward.";
  ...
  if (
    result.action?.type === "book_flow" ||
    result.action?.type === "check_availability"
  ) {
    const data = extractBookingDetails(
      input.message,
      input.sessionContext.pending_action?.data,
    );
    updatedContext.pending_action = { type: result.action.type, data };
  } else if (result.action?.type === "magic_content") {
    const data = extractMagicDetails(
      input.message,
      input.sessionContext.pending_action?.data,
    );
