 ...
 * Uses LangGraph to scan OTAs via SerpAPI Google Hotels (Tavily fallback) and beat competitors by 6%
 */
...
/**
 * Step 1: Scan OTAs using SerpAPI Google Hotels (Tavily fallback)
 */
async function scanOTAs(state: typeof PriceScoutAnnotation.State) {
  debugLog(
    `[PriceScout] Iteration ${state.iteration}: Scanning OTAs for "${state.roomType}"...`,
  );
  ...
  // Find lowest
  const lowestEntry = Object.entries(scannedPrices).reduce(
  ...
  debugLog(
    `[PriceScout] Scanned ${Object.keys(scannedPrices).length} OTAs (${liveSources} live). Best: $${lowestEntry[1]} on ${lowestEntry[0]}`,
  );
...
async function calculateBeatPrice(state: typeof PriceScoutAnnotation.State) {
  ...
  const savingsPercent =
    state.bestPrice > 0
      ? Math.round((savings / state.bestPrice) * 100 * 10) / 10
      : 6;
...
async function refineSearch(state: typeof PriceScoutAnnotation.State) {
  ...
  if (Math.random() > 0.7) {
    ...
    debugLog(
      `[PriceScout] No better deals found. Continuing to next iteration.`,
    );
    return state;
  }
}
...
export async function runPriceScout(
  ...
  debugLog(
    `✅ [PriceScout] Complete. Best OTA: ${finalState.bestOTA} @ $${finalState.bestPrice}`,
  );
  debugLog(
    `💰 [PriceScout] Direct booking price: $${finalState.beatPrice} (Save 6%!)`,
  );

  const actualSavings =
    Math.round((finalState.bestPrice - finalState.beatPrice) * 100) / 100;
  const actualSavingsPercent =
    finalState.bestPrice > 0
  ...
  return {
    ...
    priceUrl:
      finalState.priceUrl ||
      `https://linapoint.com/book?check_in=${checkInDate}&check_out=${checkOutDate}&guests=2&room_type=${encodeURIComponent(roomType)}`,
