          ...
          {!isLogin && (
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Use your WhatsApp number in E.164 format.
              </p>
            </div>
          )}
          ...
          {!isLogin && (
            <div>
              ...
              {specialEvents.map((ev, idx) => (
                <div key={idx} className="flex gap-2 mb-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setSpecialEvents(
                        specialEvents.filter((_, i) => i !== idx),
                      )
                    }
              ...
              <button
                type="button"
                onClick={() =>
                  setSpecialEvents([...specialEvents, { name: "", date: "" }])
                }
                ...
                {["Ruins", "Cuisine", "Wellness", "Kundalini"].map((item) => (
                  <label key={item} className="inline-flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked)
                          setMayaInterests([...mayaInterests, item]);
                        else
                          setMayaInterests(
                            mayaInterests.filter((m) => m !== item),
                          );
                      }}
                      disabled={loading}
                    />
          ...
          {!isLogin && (
            <div className="flex items-center gap-2">
              <input
                ...
                disabled={loading}
              />
              <label htmlFor="optInMagic" className="text-sm text-gray-700">
                Opt in to magic recommendations
              </label>
            </div>
          )}
