...
export async function POST(req: Request) {
  try {
    ...
    if (!useStripe) {
      const squareAttempt = await trySquarePayment(amount, currency, { ...metadata, user_id: user.id }, sourceId)
    ...
    // Fallback to Stripe
    const stripeResult = await tryStripePayment(amount, currency, { ...metadata, user_id: user.id })
...
async function getSquareLocationId(client: any): Promise<string | null> {
  ...
  try {
    ...
    if (active) {
      ...
      if (!isProd) console.log('[Square] Resolved location:', active.name, active.id)
      return active.id
    }
  } catch (err) {
...
async function trySquarePayment(
  ...
  try {
    ...
    if (!sourceId) {
      return { success: false, error: 'No card token (sourceId) provided for Square payment' }
    }
    ...
    const message = err?.errors?.[0]?.detail || err?.message || 'Square payment failed'
