...
export async function POST(request: NextRequest) {
  try {
    ...
    const sessionMessages =
      (existingSession?.last_messages as any) || existingContext.messages || [];
      ...
      if (!data.checkInDate || !data.checkOutDate) {
        ...
        if (priceRunId) {
          try {
            ...
            console.warn(
              "[WhatsApp] Failed to finalize PriceScout run:",
              logError,
            );
          }
        }
        ...
        try {
          curatorRunId = await createAgentRun(supabase as any, {
            ...
            input: {
              ...
              tourBudget: data.tourBudget || 500,
            },
          });
        } catch (logError) {
        ...
        if (curatorRunId) {
          try {
            ...
            console.warn(
              "[WhatsApp] Failed to finalize Curator run:",
              logError,
            );
          }
        }
    ...
    if (pending?.type === "magic_content") {
      ...
      if (!data.reservationId || !data.occasion) {
        ...
        const contentResult = await generateMagicContent(
          supabase as any,
          {
        ...
        const songUrl = contentResult.items.find(
          (item) => item.contentType === "song",
        )?.mediaUrl;
        const videoUrl = contentResult.items.find(
      ...
      if (!data.checkInDate || !data.checkOutDate) {
        ...
        try {
          ...
          const lines = avail
            .map(
              (a) =>
                `${a.available ? "✅" : "❌"} ${a.label}: ${a.availableRooms} left — $${a.baseRate}/night`,
            )
            .join("\n");
    ...
    if (pending?.type === "book_tour" && !actionHandled) {
      ...
      let tourName = "Tour";
      if (lower.includes("snorkel") || lower.includes("hol chan"))
        tourName = "Hol Chan Snorkeling";
      else if (lower.includes("fish")) tourName = "Sport Fishing";
      else if (lower.includes("ruin") || lower.includes("mayan"))
        tourName = "Mayan Ruins Day Trip";
      else if (lower.includes("cenote")) tourName = "Cenote Swimming";
      else if (lower.includes("kayak")) tourName = "Mangrove Kayaking";
      else if (lower.includes("dive") || lower.includes("blue hole"))
        tourName = "Blue Hole Diving";
      else if (lower.includes("island"))
        tourName = "Island Hopping & Beach Picnic";
    ...
    if (pending?.type === "trip_planner" && !actionHandled) {
      ...
      if (!data.checkInDate || !data.checkOutDate) {
        ...
        try {
          ...
          replyText =
            "I had trouble generating your itinerary. Let me try again — send your dates once more.";
        }
      }
    }
    ...
    if (trimmedContext.messages.length > 0) {
      const lastMessage =
        trimmedContext.messages[trimmedContext.messages.length - 1];
