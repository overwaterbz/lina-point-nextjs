...
export async function POST(request: NextRequest) {
  const limited = checkRateLimit(rateLimitKey(request), 5); // 5 req/min
  ...
  if (
    ...
    bookingAmount < Number(promo.min_booking_amount)
  ) {
    return NextResponse.json({
      valid: false,
  ...
  if (promo.room_type && roomType && promo.room_type !== roomType) {
    return NextResponse.json({
      valid: false,
      error: "This promo code is not valid for the selected room type",
    });
  }
  ...
  if (promo.discount_type === "fixed") {
    ...
    discount =
      Math.round(bookingAmount * (Number(promo.discount_value) / 100) * 100) /
      100;
