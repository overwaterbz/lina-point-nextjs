...
import { handleServiceError } from "@/shared/lib/errorHandler";
    ...
    profiles?.forEach((profile) => {
      ...
      if (profile.birthday) {
        try {
          ...
          if (
            birthDate.getUTCMonth() + 1 === todayMonth &&
            birthDate.getUTCDate() === todayDay
          ) {
      ...
      if (profile.anniversary) {
        try {
          ...
          if (
            anniversaryDate.getUTCMonth() + 1 === todayMonth &&
            anniversaryDate.getUTCDate() === todayDay
          ) {
