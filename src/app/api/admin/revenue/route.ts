...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: recentRes } = await supabase
      .from("reservations")
      .select(
        "id, room_type, total_amount, status, payment_status, check_in_date",
      )
    ...
    const totalRoomRevenue = confirmed.reduce(
    ...
    const tourRevenue = tours.reduce(
      (sum, t) => sum + (Number(t.price) || 0),
      0,
    );
    const tourCommission = tours.reduce(
    ...
    const upsellAccepted = upsellData.filter(
      (u) => u.status === "accepted",
    ).length;
    ...
    return NextResponse.json({
      ...
      revenue30d: {
        ...
        total:
          Math.round((totalRoomRevenue + tourRevenue + upsellRevenue) * 100) /
          100,
      },
      reservations30d: {
