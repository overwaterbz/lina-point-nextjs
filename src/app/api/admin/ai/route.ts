...
export async function POST(request: NextRequest) {
  ...
  if (!promptId || !['approve', 'reject'].includes(action)) {
    return NextResponse.json({ error: 'Invalid request: need promptId and action (approve|reject)' }, { status: 400 });
  }
  ...
  if (!success) {
    return NextResponse.json({ error: 'Failed to update prompt — may not be in pending_review state' }, { status: 400 });
  }
