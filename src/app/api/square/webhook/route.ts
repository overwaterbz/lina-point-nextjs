...
export async function POST(req: Request) {
  ...
  if (signatureKey) {
    ...
    console.warn(
      "[Square Webhook] No SQUARE_WEBHOOK_SIGNATURE_KEY configured — rejecting in production",
    );
    return NextResponse.json({ received: false }, { status: 400 });
  }
    ...
    if (bookingId) {
      try {
        ...
        if (existing && existing.length > 0) {
          if (!isProd)
            console.log(
              `[Square Webhook] Booking ${bookingId} already paid, skipping`,
            );
          return NextResponse.json({ received: true });
        }
        ...
        if (updateErr) {
          console.warn(
            "[Square Webhook] Failed to update tour_bookings:",
            updateErr.message,
          );
        } else if (!isProd) {
          console.log(
            `[Square Webhook] Marked tour_bookings paid for ${bookingId}`,
          );
        }
...
async function sendBookingConfirmation(
  ...
  try {
    ...
    if (!isProd)
      console.log(`[Square Webhook] Confirmation email sent to ${user.email}`);
  } catch (err) {
