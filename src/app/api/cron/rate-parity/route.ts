...
export async function GET(request: NextRequest) {
  ...
  for (const [roomType, rates] of ratesByType) {
    ...
    for (const daysOut of dateWindows) {
      ...
      try {
        ...
        console.error(
          `[RateParity] Error checking ${roomType} (${daysOut}d):`,
          err,
        );
      }
    }
  ...
  debugLog(
    `[RateParity] Checked ${results.length} windows, ${alerts.length} alerts, ${totalStored} rates stored`,
  );
