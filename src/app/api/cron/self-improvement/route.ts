...
import { runSelfImprovementAndPersist, type SelfImproveInputs } from '@/lib/agents/selfImprovementAgent';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [logsSummary, bookingSummary, prefsSummary, conversionSummary] = await Promise.all([
      gatherAgentLogs(),
    ...
    await supabase.from('agent_runs').insert({
      ...
      status: 'success',
      input: JSON.stringify({ summaryLengths: {
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Self-improvement failed' },
      { status: 500 },
    );
  }
}
...
async function gatherAgentLogs(): Promise<string> {
  ...
  const byAgent = new Map<string, { success: number; error: number; errors: string[] }>();
  for (const run of data) {
    const entry = byAgent.get(run.agent_name) || { success: 0, error: 0, errors: [] };
  ...
  const lines = Array.from(byAgent.entries()).map(([name, stats]) => {
    const errMsg = stats.errors.length ? ` Errors: ${stats.errors.join('; ')}` : '';
...
async function gatherBookingSummary(): Promise<string> {
  ...
  const byRoom = new Map<string, { count: number; revenue: number; statuses: Record<string, number> }>();
  for (const r of data) {
    const entry = byRoom.get(r.room_type) || { count: 0, revenue: 0, statuses: {} };
    entry.count++;
  ...
  const lines = Array.from(byRoom.entries()).map(([room, stats]) => {
    const statusStr = Object.entries(stats.statuses).map(([s, c]) => `${s}:${c}`).join(', ');
...
async function gatherPreferencesSummary(): Promise<string> {
  ...
  for (const p of data) {
    if (p.travel_style) styles.set(p.travel_style, (styles.get(p.travel_style) || 0) + 1);
    if (p.loyalty_tier) tiers.set(p.loyalty_tier, (tiers.get(p.loyalty_tier) || 0) + 1);
  }

  return `Guest preferences (${data.length} profiles with AI data):\nTravel styles: ${Array.from(styles.entries()).map(([s, c]) => `${s}:${c}`).join(', ')}\nLoyalty tiers: ${Array.from(tiers.entries()).map(([t, c]) => `${t}:${c}`).join(', ')}`;
}
...
async function gatherConversionSummary(): Promise<string> {
  ...
  if (promos?.length) {
    ...
    parts.push(`Promo codes: ${activePromos.length} active, ${totalUsage} total uses`);
  }

  if (campaigns?.length) {
    const byType = new Map<string, number>();
    for (const c of campaigns) byType.set(c.campaign_type, (byType.get(c.campaign_type) || 0) + 1);
    parts.push(`Recent campaigns: ${Array.from(byType.entries()).map(([t, c]) => `${t}:${c}`).join(', ')}`);
  }

  return parts.length ? `Conversion data:\n${parts.join('\n')}` : 'No conversion data available yet.';
}
