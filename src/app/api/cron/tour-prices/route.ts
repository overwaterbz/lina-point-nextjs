...
export async function GET(request: NextRequest) {
  ...
  if (toursErr || !tours?.length) {
    return NextResponse.json({ ok: true, message: 'No active tours to scrape' })
  }
  ...
  console.log(`[TourPrices] Scraped ${tours.length} tours, ${toursWithPrices} with prices, ${totalStored} rows stored`)
