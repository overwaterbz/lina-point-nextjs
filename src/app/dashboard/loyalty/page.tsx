...
const TIER_THRESHOLDS = [
  ...
  },
  {
    ...
    discount: 5,
    perks: ["5% off all stays", "1 pt per $1"],
  },
  {
    name: "Loyal",
    min: 2000,
    ...
    discount: 10,
    perks: ["10% off all stays", "1.5x points", "Free early check-in"],
  },
  {
    name: "VIP",
    min: 5000,
...
export default function LoyaltyPage() {
    ...
    (async () => {
      try {
        const [profileRes, rewardsRes, referralsRes] = await Promise.all([
          ...
          supabase
            .from("referrals")
            .select("id, referred_email, status, points_earned, created_at")
  ...
  const handleRedeem = async (reward: Reward) => {
    if (!user || !loyalty || loyalty.points < reward.points_cost) {
    ...
    const { error: deductErr } = await supabase
      .from("profiles")
      .update({ loyalty_points: loyalty.points - reward.points_cost })
      .eq("user_id", user.id);
    if (deductErr) {
    ...
    if (insertErr) {
      // Points were already deducted — restore them to keep state consistent
      await supabase
        .from("profiles")
        .update({ loyalty_points: loyalty.points })
        .eq("user_id", user.id);
      toast.error("Redemption failed — your points have been restored");
      return;
    }
    // Notify staff
    await supabase.from("notifications").insert({
      type: "loyalty_redemption",
      title: "Loyalty Redemption Request",
      message: `Guest (${user.email}) redeemed ${reward.points_cost} pts for: ${reward.name}`,
      user_id: null,
      read: false,
    });
    setLoyalty({ ...loyalty, points: loyalty.points - reward.points_cost });
    toast.success(
      `Redeemed: ${reward.name}! A staff member will reach out to arrange details.`,
    );
  };
  ...
  const currentTier =
    TIER_THRESHOLDS.findLast((t) => (loyalty?.points || 0) >= t.min) ||
    TIER_THRESHOLDS[0];
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-gray-600 mt-1">
          Earn points for every stay, tour, and referral
        </p>
      </header>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-6">
          ...
          <div>
            <span
              className={`inline-block px-3 py-1.5 rounded-full text-sm font-semibold ${currentTier.color}`}
            >
              {currentTier.name} Member
            </span>
            {nextTier && (
              <p className="text-xs text-gray-400 mt-1">
                {nextTier.min - (loyalty?.points || 0)} more points to{" "}
                {nextTier.name}
              </p>
            )}
          </div>
          ...
          <div className="mt-4">
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{
                  width: `${Math.min(100, ((loyalty?.points || 0) / nextTier.min) * 100)}%`,
                }}
              />
            </div>
          </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <p className="text-sm text-gray-600 mb-4">
          Share your code and earn 250 points when your friend completes their
          first stay!
        </p>
          ...
          <div className="mt-4 space-y-2">
            ...
            {referrals.map((r) => (
              ...
              >
                ...
                >
                  {r.status}{" "}
                  {r.points_earned > 0 && `(+${r.points_earned} pts)`}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <p className="text-sm text-gray-600 mb-4">
          Book direct to save 6% below any OTA — plus unlock loyalty discounts
          up to 15% off.
        </p>
          ...
          {TIER_THRESHOLDS.map((tier) => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span
                  className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${tier.color}`}
                >
                  {tier.name.toLowerCase() ===
                  (loyalty?.tier?.toLowerCase() || "new")
                    ? "► "
