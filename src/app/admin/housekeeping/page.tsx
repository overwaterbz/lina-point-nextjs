...
export default function HousekeepingPage() {
    ...
    try {
      const [{ data: taskData }, { data: staffData }] = await Promise.all([
        ...
        supabase
          .from("profiles")
          .select("id, full_name")
          .in("role", ["front_desk", "manager", "admin"])
          .order("full_name"),
      ]);
      const mapped = (taskData || []).map((t: Record<string, unknown>) => ({
        ...t,
        room_number:
          (t.rooms as { room_number: string } | null)?.room_number || null,
      })) as Task[];
      setTasks(mapped);
  ...
  const moveTask = async (task: Task, newStatus: string) => {
    const res = await fetch(`/api/admin/housekeeping?id=${task.id}`, {
      method: "PATCH",
  ...
  const assignTask = async (taskId: string, staffId: string) => {
    const res = await fetch(`/api/admin/housekeeping?id=${taskId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    fetchTasks();
    toast.success(staffId ? "Staff assigned" : "Assignment cleared");
  };
      ...
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Track cleaning, maintenance, and room preparation
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {STATUSES.map((status) => (
            <div
              key={status}
              className="bg-slate-100 rounded-lg p-4 min-h-[300px]"
            >
                ...
                {(tasksByStatus[status] || []).map((task) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span
                        className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${PRIORITY_STYLES[task.priority] || "bg-gray-100 text-gray-600"}`}
                      >
                        {task.priority}
                      </span>
                    </div>
                    ...
                    <div className="mb-2">
                      <select
                        ...
                        className="w-full text-[10px] border border-gray-200 rounded px-1.5 py-0.5 text-gray-700 bg-white"
                      >
