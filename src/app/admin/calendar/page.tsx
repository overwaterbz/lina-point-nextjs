    ...
    (async () => {
      try {
        const { data } = await supabase
          .from("reservations")
          .select(
            "id, confirmation_number, room_type, room_number, check_in, check_out, status, guest_name, num_guests",
          )
          .gte("check_out", start)
          .lte("check_in", end)
          .order("check_in");
    ...
    <div className="space-y-6">
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Property-wide availability and booking timeline
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow overflow-x-auto">
                      ...
                      return (
                        ...
                        >
                          {active.length > 0 ? (
                            ...
                            <div
                              className="h-5 rounded bg-green-100"
                              title="Available"
                            />
                          )}
                        </td>
          ...
          <div className="flex items-center gap-4 p-3 border-t text-xs text-gray-500">
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-green-100" /> Available
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-green-500" /> Confirmed
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-blue-500" /> Checked In
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-yellow-500" /> Pending
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-gray-400" /> Past
            </span>
          </div>
        </div>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="bg-white rounded-lg shadow p-5">
          ...
          {reservations.filter(
            (r) => r.check_in === today.toISOString().split("T")[0],
          ).length === 0 ? (
        ...
        <div className="bg-white rounded-lg shadow p-5">
          ...
          {reservations.filter(
            (r) => r.check_out === today.toISOString().split("T")[0],
          ).length === 0 ? (
