...
function StatCard({ label, value, sub, color = '#0d9488' }: { label: string; value: string | number; sub?: string; color?: string }) {
  return (
...
export default function RevenueDashboardPage() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center">
        <div className="bg-white rounded-xl shadow-lg p-8 max-w-md text-center">
          <p className="text-red-500 font-semibold text-lg">Failed to load dashboard</p>
          <p className="text-gray-500 text-sm mt-2">{error}</p>
          <button onClick={fetchData} className="mt-4 px-6 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700">
            Retry
          </button>
        </div>
      </div>
  ...
  const roomChartData = Object.entries(data.roomBreakdown).map(([type, info]) => ({
  ...
  const revenueSegments = [
    ...
    { label: 'Tour Commission', value: data.revenue30d.tourCommission, color: '#7c3aed' },
    { label: 'Upsell Revenue', value: data.revenue30d.upsells, color: '#d97706' },
  ]
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Live data as of {new Date(data.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}
            </p>
          </div>
          <div className="flex items-center gap-3">
            {loading && <div className="animate-spin h-4 w-4 border-2 border-teal-500 border-t-transparent rounded-full" />}
            <a
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard
            ...
            color={data.occupancy.pct > 90 ? '#dc2626' : data.occupancy.pct > 70 ? '#d97706' : '#0d9488'}
          />
          <StatCard
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold text-gray-900">Today&apos;s Occupancy</h2>
            <span className="text-2xl font-bold" style={{ color: data.occupancy.pct > 90 ? '#dc2626' : '#0d9488' }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Revenue Breakdown */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Revenue Breakdown (30d)</h2>
            <BarChart data={revenueSegments} maxValue={maxRevSegment} />
            <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
              <span className="text-sm font-semibold text-gray-700">Total</span>
              <span className="text-xl font-bold text-teal-600">${data.revenue30d.total.toLocaleString()}</span>
            </div>
          </div>

          {/* Room Type Performance */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Revenue by Room Type (30d)</h2>
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(data.roomBreakdown).map(([type, info]) => (
                  <div key={type} className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full" style={{ backgroundColor: ROOM_COLORS[type] || '#6b7280' }} />
                    <span className="text-xs text-gray-600">{ROOM_LABELS[type] || type}: {info.count} bookings</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Upsell Performance */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Upsell Performance</h2>
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-green-600">{data.upsells.accepted}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Conversion</span>
                <span className="font-semibold" style={{ color: data.upsells.conversionPct > 20 ? '#059669' : '#d97706' }}>
                  {data.upsells.conversionPct}%
                </span>
              </div>
              <div className="pt-3 border-t border-gray-100 flex justify-between">
                <span className="text-sm font-semibold text-gray-700">Revenue</span>
                <span className="text-lg font-bold text-amber-600">${data.upsells.revenue.toLocaleString()}</span>
              </div>
            </div>
          </div>
          ...
          {/* Reservation Funnel */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Booking Funnel (30d)</h2>
            <div className="space-y-2">
              {[
                { label: 'Total', value: data.reservations30d.total, color: '#6b7280', pct: 100 },
                { label: 'Confirmed', value: data.reservations30d.confirmed, color: '#2563eb', pct: data.reservations30d.total > 0 ? (data.reservations30d.confirmed / data.reservations30d.total) * 100 : 0 },
                { label: 'Paid', value: data.reservations30d.paid, color: '#059669', pct: data.reservations30d.total > 0 ? (data.reservations30d.paid / data.reservations30d.total) * 100 : 0 },
              ].map((step) => (
                <div key={step.label}>
              ...
              {data.reservations30d.cancelled > 0 && (
                <p className="text-xs text-red-500 mt-2">{data.reservations30d.cancelled} cancelled</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900">{data.whatsapp.messages30d}</p>
                  <p className="text-xs text-gray-500">WhatsApp messages (30d)</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center text-2xl">👑</div>
                <div>
                  <p className="text-2xl font-bold text-gray-900">{data.guests.loyal}</p>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900">{data.upsells.offered}</p>
                  <p className="text-xs text-gray-500">Upsell offers delivered</p>
                </div>
              </div>
            </div>
          </div>
