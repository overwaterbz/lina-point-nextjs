...
export default function AdminToursPage() {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editing) {
      const res = await fetch(`/api/admin/tours?id=${editing.id}`, {
        method: "PATCH",
  ...
  const handleToggleActive = async (tour: Tour) => {
    const res = await fetch(`/api/admin/tours?id=${tour.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ active: !tour.active }),
    });
    if (!res.ok) {
      toast.error("Failed to toggle tour");
      return;
    }
    fetchTours();
  };
      ...
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Manage Dee Kay&apos;s Tours catalog
          </p>
        </div>
        <button
          onClick={() => {
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <form onSubmit={handleSave} className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select
                name="category"
                defaultValue={editing?.category || "water"}
                className="w-full border rounded-lg px-3 py-2 text-sm"
              >
                {CATEGORIES.map((c) => (
            ...
            <div>
              ...
              <input
                name="max_guests"
                type="number"
                defaultValue={editing?.max_guests || 10}
                required
                className="w-full border rounded-lg px-3 py-2 text-sm"
              />
            </div>
                    ...
                    <tr className="border-t hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        {otaPrices.length > 0 ? (
                          <button
                            onClick={() =>
                              setShowOta(showOta === tour.id ? null : tour.id)
                            }
                            className="flex items-center gap-1.5"
                          >
                            <span
                              className={`inline-block w-2 h-2 rounded-full ${beating ? "bg-green-500" : "bg-red-500"}`}
                            />
                        ...
                        <td colSpan={7} className="px-4 py-2 bg-slate-50">
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                            {otaPrices.map((p, i) => (
                              <div
                                key={i}
                                className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border text-xs"
                              >
                                <div>
