...
function createCronRequest(secret?: string): NextRequest {
  ...
  return new NextRequest(new URL('http://localhost:3000/api/cron/run-daily-marketing'), {
    method: 'GET',
    headers,
  });
}
...
describe('GET /api/cron/run-daily-marketing', () => {
  ...
  it('should reject requests without valid cron secret', async () => {
    const mockDenied = { status: 401, json: async () => ({ error: 'Unauthorized' }) };
  ...
  it('should process scheduled campaigns and call runMarketingCrew', async () => {
    ...
    mockFrom.mockReturnValue({
      select: jest.fn().mockReturnValue({
        eq: jest.fn().mockReturnValue({
          gte: jest.fn().mockResolvedValue({ data: [mockCampaign], error: null }),
        }),
      }),
    ...
    expect(mockRunMarketingCrew).toHaveBeenCalledTimes(1);
  });
