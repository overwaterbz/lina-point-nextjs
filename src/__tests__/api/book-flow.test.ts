...
function setupHappyPath() {
  ...
  mockRunExperienceCurator.mockResolvedValue({
    tours: [
      {
        name: "Snorkeling at Hol Chan",
        type: "water",
        price: 75,
        duration: "3h",
        url: "https://tours.example.com/snorkel",
      },
      {
        name: "Cave Tubing",
        type: "adventure",
        price: 120,
        duration: "6h",
        url: "https://tours.example.com/cave",
      },
    ],
    totalPrice: 195,
...
describe("POST /api/book-flow", () => {
  ...
  describe("Authentication", () => {
    it("should return pricing for unauthenticated requests (no reservation created)", async () => {
      mockGetUser.mockResolvedValue({
        data: { user: null },
      ...
      // Unauthenticated users receive pricing data; reservation/payment are skipped
      expect(response.status).toBe(200);
      const data = await response.json();
      expect(data.curated_package).toBeDefined();
